define(['app'], function (app) {
	...
	app.factory('assistantDataService',assistantDataService);

	app.directive('blink', function($timeout, $rootScope) {
		return {
			restrict: 'E',
			transclude: true,
			template: '<span ng-transclude></span>',
			replace: true,
			scope: {analyzingVoiceSearchQuery:"="},
			link: function($scope, $element, attrs) {
				$scope.enableBlink = false;
				function showElement() {
					$element.css("display", "inline");
					if($scope.enableBlink) {
				...
				$rootScope.$watch("analyzingVoiceSearchQuery", function(newVal, oldVal) {
					if(newVal && newVal != oldVal) {
						$scope.enableBlink = true;
						showElement();
					} else {
						$scope.enableBlink = false;
						hideElement();
	...
	app.directive('speechDir', function (assistantDataService, $timeout, $http, $compile,$rootScope) {
		return {
			restrict: 'E',
			replace:true,
			template: '<a href="" ng-if="activateAssistant" ng-click="showSpeechModal()"> <i class="fa fa-microphone"></i>Trade With Voice</a>',
			link: function($scope, element, attrs) {
				...
				$scope.modalOptions = {
					...
					headerText: 'Connect2India Global Trade Assistant',
					bodyText: '',
					closeModal : function() {
				...
				$scope.setSpeechSearch = function(isStarted) {
					...
					$scope.speechHeaderText = 'Connect2India Global Trade Assistant';
					$scope.speechInputPlaceholder = isStarted ? 'Speaking....' : 'Tap on the mic and start speaking. eg, I want to export rice.';
				...
				$scope.searchByVoice = function() {
					...
					$rootScope.analyzingVoiceSearchQuery=true;
					assistantDataService.recognizeSearchPattern(payload).then(function(resp) {
						...
						$rootScope.analyzingVoiceSearchQuery=false;
					});
				};
