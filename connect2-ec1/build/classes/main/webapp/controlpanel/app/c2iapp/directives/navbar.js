...
define(['app'], function (app) {
	...
	var injectParams = ['$compile', '$parse', '$filter', '$rootScope'];
    var navBar = function ($compile, $parse, $filter) {
        return {
			replace: true,
			restrict: 'E',
			scope: true,
			templateUrl: '/controlpanel/app/c2iapp/views/nav.html',
			link: function(scope, el, attr) {

				scope.$watch('navEvaluated', function(newVal, oldVal) {
					console.log('navEvaluated', newVal, oldVal);
					if(newVal && !scope.naveJsExd) {
