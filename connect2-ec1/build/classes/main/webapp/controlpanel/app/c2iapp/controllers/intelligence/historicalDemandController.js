...
define(['app'], function (app) {
    ...
    var historicalDemandController = function ($scope, $location, $routeParams, authService, dataService, $rootScope, $http, $filter) {
		...
		$scope.setFilters = function (plan) {
			if('ENTERPRISE' === plan) {
				$scope.tradeDurations = [
					{value: '2018,2018', label:'Last Year'},
					{value: '2017,2018', label:'Last 2 Years'},
					{value: '2014,2018', label:'Last 5 Years'}
				];
				$scope.tradeYears = [2018, 2017, 2016, 2015, 2014, 2013, 2012, 2011];
			} else if('MULTIPLIER' === plan) {
				$scope.tradeDurations = [
					{value: '2018,2018', label:'Last Year'},
					{value: '2017,2018', label:'Last 2 Years'},
					{value: '2014,2018', label:'Last 5 Years'}
				...
				$scope.tradeYears = [2018];
			} else if ('ADVANCED' === plan) {
				$scope.tradeDurations = [
					{value: '2018,2018', label:'Last Year'},
					{value: '2017,2018', label:'Last 2 Years'}
				];
				$scope.tradeYears = [2018, 2017, 2016, 2015];
			}
		};
		...
		$scope.fetchHistoricalDemandByProductCode = function () {
			...
			dataService.getHistoricalTradeCountrywise($scope.searchCriteria.product, $scope.searchCriteria.productName, 'All', $scope.type)
			.then(function(data){
		...
		$scope.fetchHistoricalDemandByCountry = function () {
			...
			$scope.filterData = {
				...
				originCountry: $scope.searchCriteria.country,
				logAction:true
			};
			...
			.then(function(data1){
				...
				$scope.products = $filter('orderBy')(data1.stats, 'value', true);
		...
		$scope.fetchHistoricalDemandByProductCodeNCountry = function (productCode, country) {
			...
			dataService.getHistoricalTradeForProductNCountryDetailed(productCode, $scope.searchCriteria.productName, country, $scope.type, 'true')
			.then(function(data){
				$scope.products = $filter('orderBy')(data.stats, 'value', true);
				$scope.products1=data.stats;
    ...
    var chartCountryOption1 = {
		...
		$scope.loadMarketTrendData = function () {
			...
			dataService.getHistoricalDemandTrend($scope.type, trendCriteria)
			.then(function(resp){
	...
	/*------------ Geo CHART RELATED------------*/
		...
		$scope.filterSearch = function () {
			...
			if('yesterday'==$scope.filterData.duration || 'last7days'==$scope.filterData.duration || 'lastMonth'==$scope.filterData.duration){
				...
				dataService.getFilteredTradeData($scope.type, $scope.filterData)
				.then(function(data){
					$scope.products = $filter('orderBy')(data, 'value', true);
		...
		$scope.changePort = function(type, value) {
			...
			if('origin' === type) {
				$scope.filterData.originPort = parseInt(value.id);
			} else {
				$scope.filterData.destinationPort = parseInt(value.id);
			}
		...
		$scope.changeDuration = function (value) {
			...
			if('yesterday'==value || 'last7days'==value || 'lastMonth'==value){
		...
		$scope.loadSummary = function() {
			if($scope.searchCriteria.product){
				dataService.getHistoricalTradeCountrywise($scope.searchCriteria.product, 'All', 'product', 'import')
				.then(function(data){
					$scope.products2=data.stats;
				});
			} else if($scope.searchCriteria.country){
				...
				dataService.getHistoricalTradeForCountry('All', $scope.searchCriteria.country, 'import')
				.then(function(data1){
					$scope.products2=data1.stats;
				});
				 
			}
			
			if($scope.searchCriteria.country1){
				dataService.getHistoricalTradeForProductNCountryList($scope.searchCriteria.productId, 'name', $scope.searchCriteria.country1, 'import')
				.then(function(data){
			...
			if($scope.searchCriteria.country2){
				dataService.getHistoricalTradeForProductNCountryList($scope.searchCriteria.productId, 'name', $scope.searchCriteria.country2, 'import')
				.then(function(data){
					$scope.products2=data;
				}); 
			}
		...
		$scope.showData = function(event, type, countryName, productId) {
    ...
    historicalDemandController.$inject = injectParams;
    app.register.controller('historicalDemandController', historicalDemandController);
