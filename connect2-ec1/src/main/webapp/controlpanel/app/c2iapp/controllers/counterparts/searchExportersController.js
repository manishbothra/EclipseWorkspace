...
define(['app'], function (app) {
    ...
    var searchExportersController = function ($scope, $location, $routeParams, authService, dataService, $rootScope, $http, $sce,$filter) {
		...
		$scope.contact=[];
		 ...
		 $scope.cloak4="Sign up or login to view more results for "+$scope.searchObj.name+" exporters";
		...
		if(!$scope.searchObj.country ) {
		 ...
		 $scope.$watch("filterData", function(newVal, oldVal){
			...
			if(newVal != oldVal && $scope.filterInitialized[$scope.selectedTab]) {
				...
				$scope.getGlobalExporters($scope.searchForm.selectedProduct1.code, $scope.searchForm.selectedProduct1.name,$scope.pageNo,$scope.pageSize);
			}
		}, true);
		...
		$scope.exportersToDownload = "pageDefault";
		...
		$scope.getGlobalExporters = function(code, productName,pageNo,pageSize){
			var ageRange="";
			var countries=[];
			var country="All";
			...
			console.log('getting global exporters for '+productName);	
			
			$rootScope.loadingData = true;
			
			dataService.getGlobalExporters1(code, productName,pageNo,pageSize, ageRange,countries,country,ownerShipType,hasRegistrationDetails,hasTradeDetails,hasShipmentDetails,tradeLocation,hsCode,sortOption,true).then(function(response){
				...
				$scope.globalExporters=data.data;
				if($scope.globalExporters.length==0){
		...
		$scope.getIndianExporters = function(code, productName,pageNo,pageSize){
			console.log('getting indian exporters for '+productName);	
			$rootScope.loadingData = true;
			dataService.getIndianExporters(code, productName,pageNo,pageSize,true).then(function(response){
				$rootScope.loadingData = false;
				var data = response.data;
				$scope.indianExporters=data;
				if($scope.indianExporters && $scope.indianExporters.length<1){
					$scope.noDataFetched=true;
				} else{
					$scope.noDataFetched=false;
				}
			...
			if(product.name) {
				...
				if($scope.productReport.code && $scope.productReport.code  > 0) {
					...
					if($scope.userCountry.toLowerCase()=="india"){
					   $scope.getGlobalExporters($scope.productReport.code, $scope.productReport.name,$scope.pageNo,$scope.pageSize);
					} else {
						$scope.getIndianExporters($scope.productReport.code, $scope.productReport.name,$scope.pageNo,$scope.pageSize);
					}
				...
				if (product && product.length  > 2) {
					...
					if($scope.userCountry.toLowerCase()=="india"){
					    $scope.getGlobalExporters(undefined, product,$scope.pageNo,$scope.pageSize);
					} else {
						$scope.getIndianExporters(undefined, product,$scope.pageNo,$scope.pageSize);
					}
				}
			}
		...
		$scope.$watch('globalExporters', function(newValue, oldValue) {
			if(newValue && newValue.length > 0 ) {
				var resCount = $scope.globalExporters.length;
		...
		$scope.$watch('indianExporters', function(newValue, oldValue) {
			if(newValue && newValue.length > 0 ) {
				var resCount = $scope.indianExporters.length;
				$scope.downloadButtonText = 'Buy ' + resCount + ' records for Rs. ' + ($scope.UNIT_PRICE * resCount);
			} else {
				$scope.downloadButtonText = 'Buy 100 records';
			}
		...
		$scope.$watch('searchForm', function(newVal, oldVal) {
			if(newVal) {
				if(newVal.selectedProduct1 && (newVal.selectedProduct1.code || newVal.selectedProduct1.name)){
					...
					if($scope.userCountry.toLowerCase()=="india"){
						$scope.getGlobalExporters(newVal.selectedProduct1.code, newVal.selectedProduct1.name,1,10);
					} else {
						$scope.getIndianExporters(newVal.selectedProduct1.code, newVal.selectedProduct1.name,1,10);
					}
				}
			}
		}, true);
		...
		$scope.orderDetails = $scope.downloadableCount + ' Overseas Exporters of ' + name;
		
		$scope.cartProduct = {name: 'Exporters', title:'Overseas Exporters of ' + name, unitPrice:$scope.UNIT_PRICE, items:[], type: 'single', bulkQty: 0, qty : 0,
			changeQuantity: function(changeType) {
				if("bulk" === changeType) {
					var tempQty = this.qty;
					this.clearItem();				
					this.type = 'bulk';
					$scope.cart.type = 'bulkExporters';
		...
		if(cartdata) {
			try{
				...
				if(cartJson && cartJson.cartItems) {				
					var cartExporters =  $filter('filter')(cartJson.cartItems, {title:'Exporters'});
					if(cartExporters) {
						$scope.cartProduct.items = cartExporters[0].items;
					}				
				}
			} catch(err) {
				window.localStorage.removeItem("cart");
		...
		$scope.addToCart = function (company, showCart) {
			...
			if ($scope.cart['type'] === 'bulkExporters') {
		...
		$scope.downloadAll = function () {
			...
			$scope.previewList = $scope.globalIExorters;
			$rootScope.showingCart = true;
			$scope.cart.clearCart();
			if($scope.globalExporters && $scope.globalExporters.length > 0) {
				angular.forEach($scope.globalExporters, function(company, index) {
			...
			$scope.orderDetails = $scope.globalExporters.length + ' Overseas Exporters of ' + name;
    ...
    searchExportersController.$inject = injectParams;


    app.register.controller('searchExportersController', searchExportersController);
