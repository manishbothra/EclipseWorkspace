...
define(['app'], function (app) {
	...
	var globalCurrentDemandController = function ($scope, $location, $routeParams, authService, dataService, $rootScope, $filter) {
		$scope.searchCriteria = {};
		$scope.reportForm = {productType:'yourProducts'};
		
	$scope.type="export";
	$scope.countryIdMap = {};
	$scope.countries = [{name: 'USA', id: 842}];
	$scope.reportForm.selectedCountry = $scope.countries[0];
	$scope.searchType = "byHSCode"; //byProductName
	$scope.pageTitle = 'Global Current Demand Analysis';
	$scope.$watch('reportForm.productType', function(newVal, oldVal) {
		if(newVal) {
			if(newVal != 'yourProducts') {
				...
				$scope.searchType = "byProductName";
			} else {
				$scope.searchType = "byHSCode";
			}
		}
	});
	$scope.$watch('reportForm', function(newVal, oldVal) {
		if(newVal && newVal != oldVal) {
			if ($scope.reportForm.selectedProduct) {
				$scope.filterData.productCode = $scope.reportForm.selectedProduct.code;
				$scope.selectedProduct = $scope.reportForm.selectedProduct.name;
				$scope.filterData.productNames = $scope.reportForm.selectedProduct.name.split(' ');
			}
		}
	}, true);
	
	$scope.loadDemandReport = function () {
		$scope.filterSearch();
	};
	
	$scope.changeDataCountry = function(value) {
		console.log('changeDataCountry:', value);
	};
	
	$scope.selectedDuration = "anyDuration";
	$scope.selectedUrlDuration = "anyDuration";
	$scope.durationParam = '&duration=anyDuration';
	...
	$scope.loadData = function() {	
		...
		$scope.currentDemandCriteria['outputType'] = 'GROUP_BY_TRADE_DATE_PORT_DATA_ALL_DETAILS';
		$scope.currentDemandCriteria['targetLocationId'] = $scope.reportForm.selectedCountry.id;
		$scope.currentDemandCriteria['productCode'] = $scope.searchCriteria.product;
		$scope.currentDemandCriteria['productNames'] = $scope.searchCriteria.productName.split(' ');
		$scope.currentDemandCriteria['populateFilterData'] = true;
		$scope.currentDemandCriteria['pageSize'] = $scope.pageSize;
		$scope.currentDemandCriteria['pageIndex'] = $scope.currentPage - 1;
		$scope.currentDemandCriteria['duration'] = $scope.selectedUrlDuration;
		
		$scope.filterData = {
				targetLocationId: $scope.reportForm.selectedCountry.id,
				outputType : "GROUP_BY_TRADE_DATE_PORT_DATA_ALL_DETAILS",
				productCode: $scope.searchCriteria.product,
				productNames: $scope.searchCriteria.productName.split(' '),
				originCountry: 0,
				pageIndex:0,
		...
		if($scope.searchCriteria.product && !($scope.searchCriteria.oport || $scope.searchCriteria.dport)){
			...
			$scope.filterData.originPortName = decodeURI($scope.searchCriteria.oport);
			...
			$scope.filterData.destinationPortName = decodeURI($scope.searchCriteria.dport);
		...
		dataService.getGlobalCurrentDemand($scope.type,$scope.currentDemandCriteria, $scope.reportForm.selectedCountry.id)
			.then(function(data1){
				...
				if(data1.filterData && data1.filtersAllowed) {
				 ...
				 $scope.mainTitle = 'Current '+$scope.type +' demand of ' + $scope.searchCriteria.productName;
		...
		authService.getControlPanelDetails().then(function(response){
			...
			if($scope.comppro.length>0){
				angular.forEach($scope.comppro, function(value, index) {
					var code = '' + value.code;
					if(code.length == 1 || code.length == 3 || code.length == 5 ) {
						code = '0' + code;
						value.code = code;
					}
				});
				...
				$scope.searchCriteria.productName = $scope.reportForm.selectedProduct.name;
				$scope.loadData();
			} else{
	...
	$scope.loadTopPortData = function () {
		...
		if ($scope.searchCriteria.product || $scope.searchCriteria.productId) {
			trendCriteria['productCode'] = $scope.searchCriteria.product || $scope.searchCriteria.productId;
		...
		dataService.getGlobalCurrentDemand($scope.type, trendCriteria, $scope.reportForm.selectedCountry.id)
			.then(function(resp){
	...
	$scope.loadMarketTrendData = function () {
	   ...
	   $scope.filterLocation = function(location) {
		...
		$scope.filterSearch = function (newPageIndex) {
			$rootScope.loadingData = true;
			$scope.mainTitle = 'Current '+$scope.type +' demand of ' + $scope.selectedProduct;
			var payload = angular.copy($scope.filterData);
			payload['pageIndex'] = 0;
			if(newPageIndex) {
				payload['pageIndex'] = newPageIndex;
			}
			if($scope.searchType == 'byProductName') {
				payload.productCode = undefined;
				payload.productNames = $scope.selectedProduct.split(' ');
			}
			dataService.getGlobalCurrentDemand($scope.type, payload, $scope.reportForm.selectedCountry.id)
				.then(function(data){
    ...
    globalCurrentDemandController.$inject = injectParams;
    app.register.controller('globalCurrentDemandController', globalCurrentDemandController);
